...
export const calcScaleNum = ({ scale, rotate, containerSizes, elementSizes, numPages }) => {
  ...
  if (!scale || scale === 'pageWidthOption' || scale === 'pageFitOption') { // calc by container size
    ...
    if (scaleNum * elemHeight > containerSizes.height || numPages > 1) { // element doesn't fit by height (or more than 1 pages)
...
function ControlsMenu ({
  file,
  numPages, currentPage, setPage, pageText, setPageText,
  ...
  return (
    <Menu inverted className="file-dossier-file-controls">
      ...
      {numPages && numPages > 1 && currentPage && <Menu.Menu position="left">
        <Menu.Item link icon="triangle left" value={currentPage - 1} onClick={setPage} disabled={currentPage <= 1}/>
        <Menu.Item link icon="triangle right" value={currentPage + 1} onClick={setPage} disabled={currentPage >= numPages}/>
        <Menu.Item>
          <Input
            name="pageText"
            value={pageText || ''}
      ...
      {scaleNum && <Menu.Menu style={{ display: 'flex', margin: 'auto' }}>
        ...
        <Menu.Item className="file-dossier-no-padding">
          <Dropdown item className="file-dossier-scale-selection" value={scaleValue || 'pageWidthOption'}
      ...
      <Menu.Menu position="right">
        ...
        {scaleNum && <Menu.Item link icon="undo" onClick={rotateFile.bind(this, 90)} disabled={!!rotateLoading} className="file-dossier-mirror-horizontal"/>}
